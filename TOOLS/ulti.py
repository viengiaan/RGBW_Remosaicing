...
def Extract_SubImages_for_BAYER_for_Multichannels(input):
    (batch, channels, h, w) = input.size()
    DEVICE = input.device
    output = torch.zeros((batch, 4 * channels, h // 2, w // 2)).to(DEVICE).float()

    for i in range(channels):
        # RGGB bayer
        # Red channel
        output[:, 0 + 4 * i, :, :] = input[:, i, 0 : h : 2, 0 : w : 2]
        # 2 Green channels
        output[:, 1 + 4 * i, :, :] = input[:, i, 0 : h : 2, 1 : w : 2]
        output[:, 2 + 4 * i, :, :] = input[:, i, 1 : h : 2, 0 : w : 2]
        # Blue channels
        output[:, 3 + 4 * i, :, :] = input[:, i, 1 : h : 2, 1 : w : 2]
