...
def crop_patch_0db(bayer_sg, qbayer_sg, qbayer_sg0, bayer, qbayer, qbayer0, patch_size, augment=False):
    # crop patch randomly
    h, w, _ = bayer.shape
    x, y = random.randrange(0, w - patch_size + 1, 2), random.randrange(0, h - patch_size + 1, 2)
    bayer, qbayer = bayer[y:y+patch_size, x:x+patch_size, :], qbayer[y:y+patch_size, x:x+patch_size, :]

    qbayer0 = qbayer0[y:y+patch_size, x:x+patch_size, :]

    bayer_sg, qbayer_sg = bayer_sg[2*y:2*(y+patch_size), 2*x:2*(x+patch_size)], qbayer_sg[2*y:2*(y+patch_size), 2*x:2*(x+patch_size)]

    qbayer_sg0 = qbayer_sg0[2*y:2*(y+patch_size), 2*x:2*(x+patch_size)]

    # bayer_sg_bic = qbayer_sg2bayer_sg(qbayer_sg)
    bayer_sg_bic = qbayer_sg2bayer_sg1(qbayer_sg)
    bayer_sg_bic = bayer_sg_bic[..., np.newaxis]
    bayer_sg = bayer_sg[..., np.newaxis]
    qbayer_sg, qbayer_sg0 = \
        qbayer_sg[..., np.newaxis], qbayer_sg0[..., np.newaxis]
    ...
    if augment:
        ...
        if rot90: 
            ...
            qbayer_sg0 = qbayer_sg0.transpose(1,0,2)

            qbayer0= qbayer0.transpose(1,0,2)
    ...
    qbayer_sg0 = ndarray2tensor(qbayer_sg0)

    qbayer0 = ndarray2tensor(qbayer0)

    return bayer_sg, qbayer_sg, qbayer_sg0, bayer, qbayer, qbayer0
...
class Remosaic(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            qbs = self.qb_0db_images[idx]
            qbayer0 = qbs
            qbayer = qbs

            bayer_sg, qbayer_sg, qbayer_sg0 = bayer.copy(), qbayer.copy(), qbayer0.copy()

            bayer_rggb, qbayer_rggb, qbayer_rggb0 = \
                bayer2rggb(bayer_sg), qbayer2rggb(qbayer_sg), qbayer2rggb(qbayer_sg0)
            
            bayer_sg, qbayer_sg, qbayer_sg0, \
            bayer_rggb, qbayer_rggb, qbayer_rggb0 \
                = crop_patch_0db(bayer_sg, qbayer_sg, qbayer_sg0, bayer_rggb, qbayer_rggb, qbayer_rggb0, self.patch_size, self.augment)
